...
class SKLeitorFuncoesFebraban {
  static clearMask(String code) {
    final String cleanCode = code.replaceAll('.', '').replaceAll(' ', '');
    return cleanCode;
  }

  static validateBoleto(String codigo) {
  ...
  static boletoBancarioCodigoBarras(String codigo) {
    final cod = clearMask(codigo);
  ...
  static boletoBancarioLinhaDigitavel(String codigo) {
    final cod = clearMask(codigo);
  ...
  static modulo11Bancario(String bloco) {
  ...
  static convertToBoletoBancarioCodigoBarras(String codigo) {
  ...
  static convertToBoletoArrecadacaoCodigoBarras(String codigo) {
    final cod = clearMask(codigo);
    var codigoBarras = '';
  ...
  static modulo11Arrecadacao(String bloco) {
    List codigo = bloco.split('').reversed.toList();
    var multiplicador = 2;
    var somatorio = codigo.fold(0, (acc, current) {
      var soma = int.parse(current) * multiplicador;
  ...
  static boletoArrecadacaoCodigoBarras(String codigo) {
    var cod = clearMask(codigo);
  ...
  static boletoArrecadacaoLinhaDigitavel(String codigo) {
    var cod = clearMask(codigo);
    ...
    var validBlocos =
        blocos.every((e) => modulo(e['num']) == int.parse(e['DV']));
    return validBlocos && validDV;
  }

  static boletoArrecadacao(codigo) {
    var cod = clearMask(codigo);
  ...
  static calcularDadosBoleto(String barcode) {
    if (validateBoleto(barcode)) {
      if (!barcode.startsWith('8')) {
        ...
        return SKLeitorDadosBoleto(valor, dataVencimento);
      } else {
        ...
        return SKLeitorDadosBoleto(value, DateTime.now());
      }
    }
    return null;
  }

  static calculateCodebar(String barcode) {
    if (validateBoleto(barcode)) {
      if (!barcode.startsWith('8')) {
        ...
        return SKLeitorDadosBoleto(valor, DateTime.now());
      }
    }
    return null;
  }
...
class SKLeitorDadosBoleto {
  String valor;
  DateTime vencimento;

  SKLeitorDadosBoleto(this.valor, this.vencimento);
